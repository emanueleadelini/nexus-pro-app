...
export default function SetupAdminPage() {
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(`Configurazione completata per ${user.email}`);
    } catch (error: any) {
      console.error("Setup Error:", error);
      setStatus('error');
      setMessage(`Errore permessi: ${error.message}. Riprova tra un istante.`);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
      <Card className="w-full max-w-md shadow-2xl border-indigo-100 bg-white rounded-[2.5rem] overflow-hidden">
        <CardHeader className="text-center bg-slate-50 pb-8 pt-10 border-b border-slate-100">
          ...
          <CardDescription className="text-slate-500 font-medium">Sblocca i permessi per AD next lab</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6 p-8">
          {!user && status === 'idle' && (
            <div className="bg-amber-50 border border-amber-100 p-5 rounded-2xl space-y-3">
              <p className="text-xs text-amber-700 font-bold">Effettua prima il login con la tua email.</p>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  <Lock className="w-3 h-3" /> Master Key
                </div>
                <Input 
                  type="text" 
                  value={setupKey} 
                  ...
                  className="bg-slate-50 h-14 rounded-2xl border-slate-200 text-center font-bold tracking-widest"
                />
              </div>
              <Button onClick={handleSetup} className="w-full bg-indigo-600 hover:bg-indigo-700 h-14 font-black uppercase tracking-widest text-xs shadow-xl shadow-indigo-200 rounded-2xl">
                Attiva Privilegi Master
              </Button>
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-center">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Account:</p>
                <p className="text-xs text-slate-900 font-black">{user.email}</p>
              </div>
            </div>
          )}
          ...
          {status === 'error' && (
            <div className="space-y-5 text-center">
              ...
              <Button onClick={() => setStatus('idle')} variant="outline" className="w-full h-12 rounded-xl font-bold border-red-100 text-red-600">Riprova</Button>
            </div>
          )}
        </CardContent>
      </Card>
