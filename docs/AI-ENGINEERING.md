# Nexus Pro - AI Engineering Specs v10.1

## 1. Genkit Integration
L'applicazione utilizza **Firebase Genkit** come layer di orchestrazione AI. Il modello predefinito è `googleai/gemini-2.5-flash` per bilanciare velocità e costi.
...
## 3. Brand DNA Mapping
I dati raccolti nella tab "Brand DNA" vengono iniettati nel prompt Handlebars per garantire coerenza strategica:
```handlebars
CONTESTO BRAND:
...
## 4. Quota Management
Il sistema monitora i `post_usati` rispetto ai `post_totali` mensili. Se il budget è esaurito, l'interfaccia di generazione AI viene disabilitata lato client per evitare costi eccessivi.
